...
public class GameObject
{
	...
	*/
	...
	// --------------- yaw ----------------
	/**
	 * Rotates this object about the world Y axis by the specified amount.
	 * <p>
	 * This operation changes the object's heading without changing its position.
	 * Positive values rotate the object in one horizontal direction and negative
	 * values rotate it in the opposite direction.
	 *
	 * @param amount the yaw rotation amount
	 */
    public void yaw(float amount) {

        Matrix4f oldRotation,
        rotAroundAvatarUp,
        newRotation;
        Vector4f oldUp;

        // System.out.println("yaw called: " + amount);

        oldRotation = new Matrix4f(this.getWorldRotation());
        rotAroundAvatarUp = new Matrix4f().rotateY(amount);
    ...
    // --------------- pitch ----------------
	/**
	 * Rotates this object about its local right axis by the specified amount.
	 * <p>
	 * Pitch is clamped between the minimum and maximum pitch limits so that the
	 * object cannot rotate too far upward or downward.
	 *
	 * @param amount the requested pitch rotation amount
	 */
    public void pitch(float amount) {

        Matrix4f oldRotation,
        rotAroundAvatarRight,
        newRotation;
        Vector4f oldRight;
		
        float delta = amount;
		// System.out.println("DOLPHIN pitch called delta: " + delta);

        // check limits 
        if (pitchAngle + delta > maxPitch) {
            delta = maxPitch - pitchAngle;
        }

        if (pitchAngle + delta < minPitch) {
            delta = minPitch - pitchAngle;
        }
		
		pitchAngle += delta;
		
		// System.out.println("DOLPHIN pitch called pitchAngle: " + pitchAngle);
		// System.out.println("DOLPHIN pitch called delta: " + delta);
        // System.out.println("DOLPHIN pitch called: " + amount);
